...
It will pass the generated bytecode to the blockchain for execution.
Once there is a reply from the blockchain, the reply will be packed into the return message of the present protocol and sent back to the requesting pallet via Polkadot Relay Chain callbacks with the same id it got the request.
The pallet which initiated the transaction will receive the reply and will match the id of the reply with the id of the request.
It will deserialize the bytecode of the reply and will transform it to the bytecode of its smart contract language (should it be different from the counterpart’s).
